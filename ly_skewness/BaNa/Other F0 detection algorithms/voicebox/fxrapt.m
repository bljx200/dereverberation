...
for iframe=1:nframe       % loop for each frame (~10 ms)
    ...
    [ipkd,vpkd]=findpeaks2(nccfd,'q');

    % Debugging: execute the line below to plot the autocorrelation peaks.
    % findpeaks2(nccfd,'q'); xlabel(sprintf('Lag = (x+%d)*%g ms',minlag-1,1000*kdsmp/fs)); ylabel('Normalized Cross Correlation'); title (sprintf('Frame %d/%d',iframe,nframe));
